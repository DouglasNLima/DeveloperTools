...
- Image resizer & compressor for reducing local images by percentage, maximum dimensions or target file size.
- Image OCR for English text extraction from local images.
- File type inspection improvements for common developer artefacts.
- Status: URL & query string helper, hashes/checksums, image converter, Image resizer & compressor and Image OCR implemented.
