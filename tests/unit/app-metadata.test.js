...
test('formats the versioned app title consistently', () => {
  assert.equal(APP_TITLE, 'Developer Tools v0.1.0 (build 47)');
...
test('keeps the visible build stamp aligned with the offline cache version', () => {
  ...
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/power-automate-email-template\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/power-automate-email-template\.ui\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/power-platform-solution\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/power-platform-display\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/power-platform-classic-workflow-package\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/power-platform-classic-workflow-package\.ui\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/mermaid-viewer\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/mermaid-viewer\.ui\.js'/);
  ...
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/pcf-development\.ui\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/pcf-development-workbench\.ui\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/text-utilities-workbench\.ui\.js'/);
  assert.match(serviceWorkerSource, /'\.\/src\/tools\/web-api-workbench\.ui\.js'/);
});
