...
test('validates the real catalogue and keeps the current visible tool count', () => {
  ...
  assert.equal(TOOL_CATALOGUE.length, 63);
  assert.equal(getVisibleTools().length, 25);
  assert.equal(getAvailableTools().length, 25);
...
test('resolves the phase 1 legacy hashes to consolidated workbench modes', () => {
  ...
  assert.deepEqual(resolveSummary('#solution-package-inspector/flows'), {
    toolId: 'solution-package-inspector',
    mode: 'flows',
